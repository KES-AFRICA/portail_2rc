...
function manageConnectedUsers(action, email = null) {
    ...
    switch (action) {
        case 'add':
            ...
            break;
        case 'remove':
            if (email && connectedUsers[email]) {
                delete connectedUsers[email];
            }
            break;
        case 'update':
            if (email && connectedUsers[email]) {
                connectedUsers[email].lastActivity = currentTime;
            }
            break;
        case 'get':
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (enteredPasswordHash === correctPasswordHash) {
            ...
            manageConnectedUsers('add', email);
...
function checkAuthentication() {
    ...
    if (authenticated === 'true' && authTime && userEmail) {
        ...
        if (sessionAge < sessionTimeout) {
            ...
            manageConnectedUsers('update', userEmail);
            // Rediriger vers le index.html
            window.location.href = 'index.html';
        } else {
            // Session expirée, nettoyer
            manageConnectedUsers('remove', userEmail);
